...
def read_pointcloud(scan_id):
    ...
    """
    plydata = trimesh.load(os.path.join('/dataset/3DSSG/3RScan/data/3RScan', scan_id, 'labels.instances.annotated.v2.ply'), process=False)
...
def read_json(split):
    ...
    selected_scans = set()
    if split == 'train' :
        selected_scans = selected_scans.union(read_txt_to_list('/dataset/3DSSG/3DSSG_subset/train_scans.txt'))
        with open("/dataset/3DSSG/3DSSG_subset/relationships_train.json", "r") as read_file:
            data = json.load(read_file)
    elif split == 'val':
        selected_scans = selected_scans.union(read_txt_to_list('/dataset/3DSSG/3DSSG_subset/validation_scans.txt'))
        with open("/dataset/3DSSG/3DSSG_subset/relationships_validation.json", "r") as read_file:
            data = json.load(read_file)
    else:
...
def read_scan_info(scan_id, mode='rgb'):
    scan_path = os.path.join("/dataset/3DSSG/3RScan/data/3RScan", scan_id)
...
if __name__ == '__main__':
    ...
    fin_all = open(os.path.join(f'/dataset/3DSSG/3RScan/data/3RScan/{args.mode}_all_quanlity.txt'), 'a')
    for i in tqdm(selected_scans):
        ...
        save_path = f'/dataset/3DSSG/3RScan/data/3RScan/{i}/multi_view'
        os.makedirs(save_path, exist_ok=True)
        # print(f'======= map pointcloud to image =========')
        class_list, class_weight = get_label('/dataset/3DSSG/3DSSG_subset/classes.txt')
